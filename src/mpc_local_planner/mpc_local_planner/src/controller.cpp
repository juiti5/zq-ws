                           ...
                           teb_local_planner::RobotFootprintModelPtr robot_model, const std::vector<teb_local_planner::PoseSE2>& via_points)
{
    ...
    nh.param("outer_ocp_iterations", outer_ocp_iterations, outer_ocp_iterations);
    setNumOcpIterations(outer_ocp_iterations);

    // further goal opions
    nh.param("force_reinit_new_goal_dist", _force_reinit_new_goal_dist, _force_reinit_new_goal_dist);
    nh.param("force_reinit_new_goal_angular", _force_reinit_new_goal_angular, _force_reinit_new_goal_angular);

    nh.param("allow_init_with_backward_motion", _guess_backwards_motion, _guess_backwards_motion);
    nh.param("force_reinit_num_steps", _force_reinit_num_steps, _force_reinit_num_steps);

    // custom feedback:
    nh.param("prefer_x_feedback", _prefer_x_feedback, _prefer_x_feedback);
    ...
    nh.param("publish_ocp_results", _publish_ocp_results, _publish_ocp_results);
    nh.param("print_cpu_time", _print_cpu_time, _print_cpu_time);
                                                                     ...
                                                                     const std::vector<teb_local_planner::PoseSE2>& via_points)
{
    ...
    nh.param("min_obstacle_dist", min_obstacle_dist, min_obstacle_dist);
    _inequality_constraint->setMinimumDistance(min_obstacle_dist);

    bool enable_dynamic_obstacles = false;
    nh.param("enable_dynamic_obstacles", enable_dynamic_obstacles, enable_dynamic_obstacles);
    _inequality_constraint->setEnableDynamicObstacles(enable_dynamic_obstacles);

    double force_inclusion_dist = 0.5;
    nh.param("force_inclusion_dist", force_inclusion_dist, force_inclusion_dist);
    double cutoff_dist = 2;
    nh.param("cutoff_dist", cutoff_dist, cutoff_dist);
