...
private:
  ...
  int linear_state_, angular_state_, brake_;

  std::string port_name_;
  double rate_;
  double linear_scale_, angular_scale_;
  double linear_init_, linear_max_, linear_step_;
  double angular_init_, angular_max_, angular_step_;
  bool send_flag_;

  bool init();
  void parseKeyboard();
  void brake(int value, int& scale);
