...
#include "keyboard_control/keyboard_control_node.h"
...
namespace autolabor_tool
{
  ...
  KeyboardControl::KeyboardControl() : linear_state_(0), angular_state_(0), port_name_("")
  {
    ...
    private_node.param("linear_init", linear_init_, 0.2);
    private_node.param("linear_max", linear_max_, 2.0);
    private_node.param("linear_step", linear_step_, 0.2);

    private_node.param("angular_init", angular_init_, 0.5);
    ...
    linear_scale_ = linear_init_;
    angular_scale_ = angular_init_;
    angular_max_;
    send_flag_ = true;
    brake_ = 0;
  }
  ...
  void KeyboardControl::buttonScaleCheck(int value, double &scale, double step, double limit)
  {
    ...
    {
      scale = scale + step;
      scale = (abs(scale) >= limit) ? (limit * scale) * limit / abs(limit * scale) : scale;
      scale = (scale <= 0) ? 0 : scale;

      //    ROS_INFO("INFO: [scale]:%.3f ,abs(scale):%.3f ,limit:%.3f ",scale,abs(scale),limit);
    }
  }

  void KeyboardControl::brake(int value, int &scale)
  {
    if (value == 2 || value == 1)
    {
      scale = 1;
...
//        break;
      ...
      if ((dir_str.find("event-kbd") < dir_str.length()) && (dir_str.find("usb") < dir_str.length()))
      {
        port_name_ = std::string(path) + "/" + dir_str;
        ROS_INFO_STREAM("INFO: The keyboard port is :" << port_name_);
        break;
