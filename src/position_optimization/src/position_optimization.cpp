...
private:
  ...
  void topic_switch_callback(const std_msgs::Bool::ConstPtr &msg);
...
private:
  ...
  ros::Publisher carpos_pub_;
  ros::Publisher uwb_pub_;
  ros::Publisher pose_init_pub_;
  
  bool publish_tf_, optimization_, trans_ok_, topic_sw_;
  std::string odom_frame_, base_frame_, map_frame_, pos_topic_;
  int control_rate_;
...
Optimization::Optimization() : position_flag_(true), real_position_(false), trans_ok_(false), topic_sw_(false)
{
...
void Optimization::topic_switch_callback(const std_msgs::Bool::ConstPtr &msg)
{
  if(msg->data){
    topic_sw_ = true;
  }
...
//  ROS_INFO("dt:%.2f",dt);
//  ROS_INFO("TIME>>=L:%.2f,N:%.2f,D:%.2f",last_time,now,dt);
  ...
  if ((abs(diff_yaw) > 0.2) && abs(vth) > 0.02)
  {
    real_yaw += -vth*dt;//last_yaw;    //*8 + curr_yaw*2)/10;
    curr_yaw = real_yaw;
    ROS_INFO("ERROR>>=VTH:%.5F,DVTH:%.2F,DY:%.2F",vth,dvth,diff_yaw);
  }
  else if (abs(vx) <= 0.01)
  {
    if((abs(diff_yaw) > 0.01) && !real_position_)
    {
      real_yaw = last_yaw; //*8 + curr_yaw*2)/10;
      curr_yaw = real_yaw;
      ROS_INFO("ERROR000[%d]=VTH:%.2F,DVTH:%.2F,DY:%.2F",real_position_,vth,dvth,diff_yaw);
    }
  }
...
//  }
  ...
  */
  ...
  geometry_msgs::Quaternion quat = tf::createQuaternionMsgFromYaw(position_yaw_);/*real_yaw*/
  /*
...
*/
  ...
  if (abs(vx) >= 0.01)
  {
    if ((distance > 0.5) && (distance < 0.8))
    {
      ...
      real_position_ = true;
//      ROS_WARN("tag ok,distance:%.2f", distance);
    ...
    {
//      ROS_WARN("tag unstable,distance:%.2f", distance);
      real_position_ = false;
    }
  }
  if (abs(vx) <= 0.01)
  {
    if ((distance > 0.65) && (distance < 0.71))
    {
      position_yaw_ = atan2(diff_y, diff_x);
      real_position_ = true;
//      ROS_WARN("tag ok,distance:%.2f", distance);
    ...
    {
//      ROS_WARN("0tag unstable,distance:%.2f", distance);
      real_position_ = false;
    }
  }

  if (count >= 10 && !trans_ok_)
  {
    start_x = p_rx / 10.0;
    start_y = p_ry / 10.0;
    start_th = p_yaw / 10.0;
    ...
    ofile << "{ 'x': " << start_x << ", 'y': " << start_y << ", 'yaw': " << start_th << " }";
    ofile.close();
  }
...
void Optimization::msg_pub_callback(const ros::TimerEvent &)
{
  ...
  pose_init_msg_.x = position_x_;
  pose_init_msg_.y = position_y_;
  pose_init_msg_.yaw = position_yaw_;
...
//    ROS_INFO("ku");
  ...
  {
    carpos_pub_.publish(pose_msg_);
//    ROS_INFO("carpos");
  }
  uwb_pub_.publish(uwb_msg_);
  check_odom_pub_.publish(check_msg_);
  pose_init_pub_.publish(pose_init_msg_);
}

void Optimization::amcl_callback(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr &msg)
{
  amcl_msg_ = *msg.get();
  ...
  br_.sendTransform(tf::StampedTransform(transform_, ros::Time::now(), "odom", "car"));
}
...
void Optimization::run()
{
  ...
  private_node.param<std::string>("pos_topic", pos_topic_, std::string("ku_pos"));
  ...
  private_node.param<std::string>("file_name", file_name_, std::string("/home/data"));
  ...
  if (init())
  {
    ...
    check_odom_pub_ = node.advertise<position_optimization::check_odom>("uwb_pos", 10);
    pose_pub_ = node.advertise<nav_msgs::Odometry>("ku_pos", 10);
    carpos_pub_ = node.advertise<nav_msgs::Odometry>("carpos", 10);
    uwb_pub_ = node.advertise<nav_msgs::Odometry>("uwb_pose", 10);
    pose_init_pub_ = node.advertise<position_optimization::pose_init>("init_pose", 10);
    ros::Subscriber amcl_sub = node.subscribe<geometry_msgs::PoseWithCovarianceStamped>("/amcl_pose", 10, &Optimization::amcl_callback, this);
    ...
    ros::Subscriber topic_switch_subscribe = node.subscribe<std_msgs::Bool>("topic_switch", 10, &Optimization::topic_switch_callback, this);
