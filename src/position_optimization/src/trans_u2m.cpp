...
geometry_msgs::Twist goal_msg_;
geometry_msgs::PoseStamped map_goal_;

bool pub_ = false;
bool once_ = false;
double trans_x_, c_x_;
double trans_y_, c_y_;
double trans_yaw_, c_yaw_;
...
void mlocal_path_callback(const nav_msgs::Path::ConstPtr &msg)
{
    /*
  mlocal_path = *msg.get();
  double _x = mlocal_path.poses.pose.position.x;
  double _y = mlocal_path.poses.pose.position.y;
  tf::Quaternion quat;
  tf::quaternionMsgToTF(mlocal_path.poses.pose.orientation, quat);
...
void mglobal_path_callback(const nav_msgs::Path::ConstPtr &msg)
{
    ...
    static int seq, seq1;
    //    int size = mglobal_path.poses.size();
    uglobal_path.poses.clear();
    for (auto i : mglobal_path.poses)
    {
        geometry_msgs::PoseStamped temp_pose;
        temp_pose.header.frame_id = mglobal_path.header.frame_id;
        ...
        //   ROS_INFO("[[seq:%d]Num:%d]==X: %.2f, Y: %.2f", seq, seq1, _x, _y);
        seq1++;
    }
    seq1 = 0;
    //    ROS_INFO("PUB");
    //    uglobal_path.poses.resize(size);
    //    int size2 = uglobal_path.poses.size();
    //    ROS_INFO("[[size1:%d]size2:%d]", size, size2);

    uglobal_path.header.frame_id = "map";
    uglobal_path.header.stamp = ros::Time::now();
    uglobal_path.header.seq = seq;
    global_path_pub_.publish(uglobal_path);
    seq++;
}
void path_callback(const ros::TimerEvent &)
{
    //    global_path_pub_.publish(uglobal_path);
}
...
void kfu_callback(const nav_msgs::Odometry::ConstPtr &msg)
{
    ...
    tf::Matrix3x3(quat).getRPY(roll, pitch, yaw);
    //    Eigen::Matrix3d u_pos = transform(_x, _y, yaw, m2px, m2py, trans_yaw_);
    Eigen::Matrix3d u_pos = transform(_x, _y, yaw, trans_x_+c_x_, trans_y_+c_y_, -trans_yaw_+c_yaw_);
...
void amcl_callback(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr &msg)
{
    amcl_msg_ = *msg.get();
    /*
    double ux = kfu_msg_.pose.pose.position.x;
    double uy = kfu_msg_.pose.pose.position.y;
    double ax = amcl_msg_.pose.pose.position.x;
    double ay = amcl_msg_.pose.pose.position.y;
    double ta = amcl_msg_.header.stamp.toSec();
    double tp = kfu_msg_.header.stamp.toSec();
    double dt = abs(ta - tp);
    ROS_INFO("dt:%.3f",dt);
    ROS_INFO("\nU[x:%.2f,y:%.2f],\nA[x:%.2f,y:%.2f]",ux,uy,ax,ay);
    if(dt < 0.05)
    {
      map_points_.emplace_back(ax, ay);
      odom_points_.emplace_back(ux, uy);
...
void pub_callback(const ros::TimerEvent &)
{
    ...
    if (distance > 0.8 || distance < 0.5 || distance == 0 || dt > 0.2)
    {
...
void goal_callback(const geometry_msgs::Twist::ConstPtr &msg)
{
    goal_msg_ = *msg.get();
    once_ = true;
    ROS_INFO("RECV:%d", pub_);
}
...
{
    double tx = trans_x_+c_x_;
    double ty = trans_y_+c_y_;
    double ro = -trans_yaw_+c_yaw_;
    if (pub_ && goal_msg_.angular.y == 3212)
    {
//        ROS_INFO("Goal_PUB:%.2f", goal_msg_.angular.y);
        //double ox = goal_msg_.pose.position.x;
        //double oy = goal_msg_.pose.position.y;
        double ox = goal_msg_.linear.x;
        double oy = goal_msg_.linear.y;
        double oyaw = goal_msg_.linear.z;
        //t
...
*/
        result_r_ = transform(ox, oy, oyaw, tx, ty, ro);
    ...
    if (pub_)
    {
        ...
        //t
            ...
            ROS_INFO("PUB:tx:%.2f, ty:%.2f, yaw:%.2f", init_x, init_y, init_yaw);
//            ROS_INFO("PUB:tx:%.2f, ty:%.2f", tx, ty);
...
int main(int argc, char **argv)
{
    ...
    private_node.param<double>("c_x", c_x_, 0);
    private_node.param<double>("c_y", c_y_, 0);
    private_node.param<double>("c_yaw", c_yaw_, 0);
    ...
    ros::Subscriber goal_sub = node.subscribe<geometry_msgs::Twist>("/car_goal_run", 10, &goal_callback);

    ros::Subscriber init_pose_sub = node.subscribe<position_optimization::pose_init>("/init_pose", 10, &init_pose_callback);

    local_path_pub_ = node.advertise<nav_msgs::Path>("/local_path", 1);
    global_path_pub_ = node.advertise<nav_msgs::Path>("/global_path", 1);
    ...
    //ros::Timer path_timer = node.createTimer(ros::Duration(0.5), &path_callback);
